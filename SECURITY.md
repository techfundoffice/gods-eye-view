...
- **No arbitrary-URL fetching.** The CCTV frame proxy fetches only server-registered camera/frame URLs — clients cannot pass an upstream URL to fetch (SSRF mitigation). Other proxies target fixed upstream hosts. `/api/youtube/live-chat` reads chat on the signed-in operator's own YouTube watch page: it fetches only `https://www.youtube.com/watch` and `youtubei/v1/live_chat/get_live_chat` for an 11-character video id, requires the YouTube session, and never returns the WEB client key.
