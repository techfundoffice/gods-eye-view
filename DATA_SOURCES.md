...
Cloud Computer AI.com's **code** is [MIT](LICENSE)-licensed. **The MIT grant covers the source code only — it does NOT extend to third-party data or visual assets.** Every third-party source keeps its own license and terms. This file documents the live and bundled data sources; bundled 3D-model provenance is recorded in [`public/models/README.md`](public/models/README.md).
...
| **YouTube live chat** (our own watch page, `get_live_chat`) | Read-only chat on the signed-in operator's own YouTube Live page (YouTube Settings + Youtube AI Comment Harness) | Same chat the YouTube player already shows on that page; [YouTube ToS](https://www.youtube.com/t/terms) apply; not a Data API quota path | YouTube — our page, read-only; no sending |
...
> If you use Cloud Computer AI.com commercially, delete `src/data/local_data/telegeography_submarine_cables/` (or obtain a commercial license from TeleGeography). It is one self-contained folder; the rest of the app runs without it.
