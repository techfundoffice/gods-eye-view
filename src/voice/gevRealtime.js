    ...
    const contextEvent = {
      ...
      item: {
        ...
        content: [
          {
            type: 'input_text',
            text: 'Current Cloud Computer AI.com viewport screenshot. Read any clearly visible street, building, and place labels in the image and combine them with the structured nearbyPlaces, streetLabels, and scene context. Do not invent labels that are not legible.',
          },
          {
            type: 'input_image',
            image_url: imageUrl,
...
function createVoiceControl({ reset = false } = {}) {
  ...
  if (!root) {
    ...
    `;
    const host = document.getElementById("left-panel-stack")
      || document.getElementById("first-run-launcher")
      || document.body;
  ...
  const leftNav = document.getElementById("left-panel-stack")
    || document.getElementById("first-run-launcher");
  if (leftNav && root.parentElement !== leftNav) {
    leftNav.insertBefore(root, leftNav.firstChild);
  }
  return {
    root,
