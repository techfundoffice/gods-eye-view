...
function createMissionOrbitPatternMaterial(color) {
  // Use a Cesium built-in rather than a process-global custom material cache
  // entry. If Vite reloads one side of the Cesium module graph, an entity can
  // otherwise retain a custom type that the active renderer no longer knows;
  // Material.fromType then throws during Scene.render and stops the whole app.
    ...
    if (!primitiveOrbitAdded) {
      _dataSource.entities.add({
        ...
        polyline: {
          ...
          width: 3,
          material: new Cesium.PolylineDashMaterialProperty({
            color: Cesium.Color.fromCssColorString('#c084fc').withAlpha(0.95),
            gapColor: Cesium.Color.TRANSPARENT,
            dashLength: 12,
            dashPattern: 0b1111111100000000,
          }),
