# Cloud Computer AI.com — Testing Guide (voice map-annotation whiteboard + live tracking)
...
- **Voice (the real feature):** click **Cloud Computer AI.com MIC** (bottom of screen) → wait for **LISTENING** →
...
1. While tracking a plane over Austin, Cloud Computer AI.com MIC: *"annotate downtown Austin."*
...
> All from the Austin default view (no camera move needed). Click Cloud Computer AI.com MIC, then speak.
...
- **No Cloud Computer AI.com MIC button / voice errors:** `OPENAI_API_KEY` didn't load — use the console API for
