...
- **Two players** sit in row 3 of `#title-bar`, directly under the tagline, side by side in a `.gev-home-video-rail` flex row that owns the grid slot. The second is **the split view** (`#gev-split-view`); today it is an exact duplicate of the first apart from audio, and is where the split-view mode will be built.
- `initHomeVideo(doc, { rootId, muted, primary })` mounts one player. Child element ids are derived as `` `${rootId}-<part>` ``, which is why the duplicate needed no new naming scheme. Live controllers live in a `Map` keyed by root id: re-mounting the *same* root replaces it (which is what `main.js` calling twice relies on), while a second player is left alone. Only the `primary` controller is reachable by `control_video_player` — a secondary player is a view, not a command target.
- **The left player has sound; the right stays muted.** Two unmuted copies of the same stream would echo a fraction of a second out of phase on the broadcast. `muted: true` skips the unmute path entirely rather than unmuting and re-muting.
- Sizes are independent per player and persist under separate keys: the primary keeps the bare `gev:home-video:v1`, others use `gev:home-video:v1:<rootId>`.
- Players are `flex: 0 0 auto` so a Medium player holds its 560 px instead of being shrunk to fit beside its sibling; the rail wraps when there is genuinely no room. Note the width settles in ~1–2 s rather than the 300 ms transition, because `#title-bar` is `width: max-content` and its grid re-layout trails the transition.
