...
- **Three video surfaces** sit in row 3 of `#title-bar`, directly under the tagline, inside the wrapping `.gev-home-video-rail`: Player 1 (`#gev-home-video`), the independent trending split view (`#gev-split-view`), and **Video Player 2** (`#gev-video-player-2`), which is a non-destructive clone of Player 1.
- `initHomeVideo(doc, { rootId, muted, primary })` mounts Player 1 and its clone. Child element ids derive from `` `${rootId}-<part>` ``, and live controllers are keyed by root id so each instance remains independent. Only Player 1 is reachable by `control_video_player`; the clone does not replace or redirect the primary command target.
- Player 1 and Video Player 2 share the same ADMIN source, recommendation, queue, sizing, and YouTube embed behavior. The clone is intentionally muted to prevent duplicate audio from the same stream. The split-view trending commentator remains a separate implementation.
- Sizes persist independently: Player 1 keeps `gev:home-video:v1`, while the clone uses `gev:home-video:v1:gev-video-player-2`.
- Players are `flex: 0 0 auto` so a Medium player holds its 560 px instead of being shrunk to fit beside another surface; the rail wraps when there is genuinely no room. Note the width settles in ~1–2 s rather than the 300 ms transition, because `#title-bar` is `width: max-content` and its grid re-layout trails the transition.
