...
- **ADMIN YouTube Go Live.** After ADMIN login, the YouTube Go Live plugin (and the core Go Live pane) captures the globe with headless Chromium and publishes H.264/AAC to YouTube over RTMP. Paste a current Studio stream key there, or create/select a broadcast from the signed-in YouTube account. Stream keys never appear on the public homepage. Optional ODBC audit persistence is degraded when no driver is installed.
