...
- **Studio stream key go-live.** The homepage panel and `/go-live.html` accept a current YouTube Studio stream key (or a full `rtmps://…/live2/…` URL) and start the globe encoder without OAuth. A Replit Secret named `YOUTUBE_STREAM_KEY` is picked up without restarting Vite.
